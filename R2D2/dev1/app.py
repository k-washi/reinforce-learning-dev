...
@ray.remote
class Actor:
    def __init__(self, pid, env_name, epsilon, gamma, eta, alpha, burnin_length, unroll_length):
    ...
    def _rollout(self) -> Tuple[list, list]:
...
class Learner:
    ...
    def __init__(self, env_name, gamma, eta, alpha, burnin_length, unroll_length):
        self.env_name = env_name
...
def main(num_actors, env_name="CartPole-v0", batch_size=16, update_iter=16, gamma=0.97, eta=0.9, alpha=0.9, burnin_length=4, unroll_length=4):
    ...
    while learner_cycles <= 50:
        ...
        if finished_learner:
            ...
            minibatchs = [replay.sample_minibatch(batch_size=batch_size) for _ in range(update_iter)]
