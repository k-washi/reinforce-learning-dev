...
class Agent():
    def __init__(self, agent_id, env_name, gamma=0.98, entropy_coef=0.01, batch_size=8):
...
def learn(num_agents=5, env_name="CartPole-v1", num_updates=50000, lr=1e-4):
    ...
    agents = [Agent.remote(agent_id=i, env_name=env_name) for i in range(num_agents)]

    weights = global_policy.get_weights()
    work_in_progresses = [agent.rollout_and_compute_grads.remote(weights) for agent in agents]
