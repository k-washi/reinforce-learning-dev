...
class DQN():
    def __init__(self, state_size, action_size, max_episode=500, replay_buffer_num=5000) -> None:
    ...
    def train(self, batch_size):
        ...
        if self._Double_DQN:
            other_q_values = self.main_net(next_states)
            max_q_actions = np.amax(other_q_values, axis=1)
        else:
            max_q_actions = np.amax(next_q_values, axis=1)
        max_q_actions = tf.one_hot(max_q_actions, self._action_size)
        max_q_values = tf.reduce_sum(max_q_actions * next_q_values, axis=1, keepdims=True)
        
        target_Qs = rewards + not_dones * self.gamma * max_q_values # (8, 1)
...
def main():
    ...
    dqn = DQN(state_size, action_size, num_epi, replay_buffer_num)
    done = False
